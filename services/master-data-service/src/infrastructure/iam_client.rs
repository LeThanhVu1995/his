...
pub async fn register_permissions(cfg: &Settings) -> anyhow::Result<()> {
    let base = cfg.iam_service_base_url.as_deref()
        .context("IAM_SERVICE_BASE_URL missing")?;
    let token = cfg.iam_service_token.as_deref()
        .context("IAM_SERVICE_TOKEN missing")?;

    let url = format!("{}/policies/register", base);
    let body = RegisterRequest { permissions: permission_catalog(&cfg.service_name) };
    ...
    if !res.status().is_success() {
        ...
        anyhow::bail!("Failed to register permissions: {}", txt);
    }
    Ok(())
}
