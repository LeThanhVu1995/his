    ...
    S::Future: 'static,
{
    ...
    fn call(&self, req: ServiceRequest) -> Self::Future {
        ...
        let user = match extensions.get::<AuthUser>() { Some(u) => u, None => return Box::pin(async move { Err(AppError::Unauthorized.into()) }) };
        let needed = self.perm;
        if !user.scopes.iter().any(|s| s == needed) { return Box::pin(async move { Err(AppError::Forbidden.into()) }); }
        drop(extensions);
