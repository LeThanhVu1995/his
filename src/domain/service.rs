...
use crate::domain::repo::{MedRepo, PrescRepo, PrescItemRepo, DispenseRepo};

pub struct PharmacyService<'a> {
    pub meds: MedRepo<'a>,
    pub presc: PrescRepo<'a>,
    pub items: PrescItemRepo<'a>,
    ...
    pub async fn create_prescription(&self, req: &crate::http::dto::prescription_dto::CreatePrescriptionReq, ordered_by: Option<&str>) -> anyhow::Result<Uuid> {
        ...
        for it in &req.items {
            items.push(PrescriptionItem {
                ...
                qty: Decimal::try_from(it.qty).unwrap_or_default(),
